...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/gabmac/Documents/RU/QP/exp/group1/exp.py',
...
for thisExp1_retrial in exp1_retrial:
    ...
    while continueRoutine:
        ...
        if exp1_instr_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp1_instr_text.started')
    ...
    while continueRoutine:
        ...
        if end_exp1_trial_text.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'end_exp1_trial_text.started')
        ...
        if end_exp1_trial_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'end_exp1_trial_key.started')
            end_exp1_trial_key.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(end_exp1_trial_key.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(end_exp1_trial_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
...
while continueRoutine:
    ...
    if exp1_start_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'exp1_start_key.started')
        exp1_start_key.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(exp1_start_key.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(exp1_start_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
    ...
    if exp1_start_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'exp1_start_text.started')
...
for thisComponent in exp1_startComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if exp1_start_key.keys in ['', [], None]:  # No response was made
    exp1_start_key.keys = None
thisExp.addData('exp1_start_key.keys',exp1_start_key.keys)
if exp1_start_key.keys != None:  # we had a response
    thisExp.addData('exp1_start_key.rt', exp1_start_key.rt)
thisExp.nextEntry()
...
for thisExp1_loop in exp1_loop:
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if exp1_image_l.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp1_image_l.started')
        ...
        if exp1_image_l.status == STARTED:
            ...
            if tThisFlipGlobal > exp1_image_l.tStartRefresh + 5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp1_image_l.stopped')
        ...
        if exp1_image_r.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp1_image_r.started')
        ...
        if exp1_image_r.status == STARTED:
            ...
            if tThisFlipGlobal > exp1_image_r.tStartRefresh + 5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp1_image_r.stopped')
...
while continueRoutine:
    ...
    if exp1_end_text.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'exp1_end_text.started')
    ...
    if exp1_end_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'exp1_end_key.started')
        exp1_end_key.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(exp1_end_key.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(exp1_end_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
...
for thisComponent in exp1_endComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if exp1_end_key.keys in ['', [], None]:  # No response was made
    exp1_end_key.keys = None
thisExp.addData('exp1_end_key.keys',exp1_end_key.keys)
if exp1_end_key.keys != None:  # we had a response
    thisExp.addData('exp1_end_key.rt', exp1_end_key.rt)
thisExp.nextEntry()
...
for thisExp2_retrial in exp2_retrial:
    ...
    while continueRoutine:
        ...
        if exp2_instr_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp2_instr_text.started')
    ...
    exp2_trial_loop = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('/Users/gabmac/Documents/RU/QP/exp/practice_trial/trial_exp_2/trial.xlsx', selection='0:8'),
    ...
    for thisExp2_trial_loop in exp2_trial_loop:
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if exp2_trial_iamge_l.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_trial_iamge_l.started')
            ...
            if exp2_trial_iamge_l.status == STARTED:
                ...
                if tThisFlipGlobal > exp2_trial_iamge_l.tStartRefresh + 5-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'exp2_trial_iamge_l.stopped')
            ...
            if exp2_trial_image_r.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_trial_image_r.started')
            ...
            if exp2_trial_image_r.status == STARTED:
                ...
                if tThisFlipGlobal > exp2_trial_image_r.tStartRefresh + 5-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'exp2_trial_image_r.stopped')
    ...
    while continueRoutine:
        ...
        if end_exp2_trial_text.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'end_exp2_trial_text.started')
        ...
        if end_exp2_trial_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'end_exp2_trial_key.started')
            end_exp2_trial_key.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(end_exp2_trial_key.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(end_exp2_trial_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
...
# keep track of which components have finished
exp2_startComponents = [exp2_start_key]
...
while continueRoutine:
    ...
    if exp2_start_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'exp2_start_key.started')
        exp2_start_key.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(exp2_start_key.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(exp2_start_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
...
for thisComponent in exp2_startComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if exp2_start_key.keys in ['', [], None]:  # No response was made
    exp2_start_key.keys = None
thisExp.addData('exp2_start_key.keys',exp2_start_key.keys)
if exp2_start_key.keys != None:  # we had a response
    thisExp.addData('exp2_start_key.rt', exp2_start_key.rt)
thisExp.nextEntry()
...
for thisExp2_loop in exp2_loop:
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if exp2_image_l.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp2_image_l.started')
        ...
        if exp2_image_l.status == STARTED:
            ...
            if tThisFlipGlobal > exp2_image_l.tStartRefresh + 5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_image_l.stopped')
        ...
        if exp2_image_r.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp2_image_r.started')
        ...
        if exp2_image_r.status == STARTED:
            ...
            if tThisFlipGlobal > exp2_image_r.tStartRefresh + 5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_image_r.stopped')
...
while continueRoutine:
    ...
    if full_end_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'full_end_text.started')
    ...
    if full_end_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'full_end_key.started')
        full_end_key.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(full_end_key.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(full_end_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
...
for thisComponent in full_endComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if full_end_key.keys in ['', [], None]:  # No response was made
    full_end_key.keys = None
thisExp.addData('full_end_key.keys',full_end_key.keys)
if full_end_key.keys != None:  # we had a response
    thisExp.addData('full_end_key.rt', full_end_key.rt)
thisExp.nextEntry()
