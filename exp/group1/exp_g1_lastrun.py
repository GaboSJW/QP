...
for thisExp1_retrial in exp1_retrial:
    ...
    for thisExp1_trial_loop in exp1_trial_loop:
        ...
        # keep track of which components have finished
        exp1_trialComponents = [exp1_trial_image_l, exp1_trial_image_r, exp1_trial_key, exp1_trial_sound, exp1_trial_cross]
        ...
        while continueRoutine and routineTimer.getTime() < 5.25:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if exp1_trial_image_l.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if exp1_trial_image_r.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            waitOnFlip = False
            if exp1_trial_key.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if exp1_trial_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                exp1_trial_cross.frameNStart = frameN  # exact frame index
                exp1_trial_cross.tStart = t  # local t and not account for scr refresh
                exp1_trial_cross.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(exp1_trial_cross, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp1_trial_cross.started')
                exp1_trial_cross.setAutoDraw(True)
            if exp1_trial_cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > exp1_trial_cross.tStartRefresh + 0.25-frameTolerance:
                    # keep track of stop time/frame for later
                    exp1_trial_cross.tStop = t  # not accounting for scr refresh
                    exp1_trial_cross.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'exp1_trial_cross.stopped')
...
for thisExp1_loop in exp1_loop:
    ...
    # keep track of which components have finished
    exp1Components = [exp1_image_l, exp1_image_r, exp1_key, exp1_sound, exp1_cross]
    ...
    while continueRoutine and routineTimer.getTime() < 5.25:
        ...
        if exp1_image_l.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        if exp1_image_r.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *exp1_key* updates
        waitOnFlip = False
        if exp1_key.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *exp1_cross* updates
        if exp1_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp1_cross.frameNStart = frameN  # exact frame index
            exp1_cross.tStart = t  # local t and not account for scr refresh
            exp1_cross.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp1_cross, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp1_cross.started')
            exp1_cross.setAutoDraw(True)
        if exp1_cross.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > exp1_cross.tStartRefresh + 0.25-frameTolerance:
                # keep track of stop time/frame for later
                exp1_cross.tStop = t  # not accounting for scr refresh
                exp1_cross.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp1_cross.stopped')
...
for thisExp2_retrial in exp2_retrial:
    ...
    for thisExp2_trial_loop in exp2_trial_loop:
        ...
        # keep track of which components have finished
        exp2_trialComponents = [exp2_trial_iamge_l, exp2_trial_image_r, exp2_trial_key, exp2_trial_sound, exp2_trial_cross]
        ...
        while continueRoutine and routineTimer.getTime() < 5.25:
            ...
            if exp2_trial_iamge_l.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if exp2_trial_image_r.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            waitOnFlip = False
            if exp2_trial_key.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if exp2_trial_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                exp2_trial_cross.frameNStart = frameN  # exact frame index
                exp2_trial_cross.tStart = t  # local t and not account for scr refresh
                exp2_trial_cross.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(exp2_trial_cross, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_trial_cross.started')
                exp2_trial_cross.setAutoDraw(True)
            if exp2_trial_cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > exp2_trial_cross.tStartRefresh + 0.25-frameTolerance:
                    # keep track of stop time/frame for later
                    exp2_trial_cross.tStop = t  # not accounting for scr refresh
                    exp2_trial_cross.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'exp2_trial_cross.stopped')
...
for thisExp2_loop in exp2_loop:
    ...
    # keep track of which components have finished
    exp2Components = [exp2_image_l, exp2_image_r, exp2_key, exp2_sound, exp2_cross]
    ...
    while continueRoutine and routineTimer.getTime() < 5.25:
        ...
        if exp2_image_l.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        if exp2_image_r.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *exp2_key* updates
        waitOnFlip = False
        if exp2_key.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *exp2_cross* updates
        if exp2_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp2_cross.frameNStart = frameN  # exact frame index
            exp2_cross.tStart = t  # local t and not account for scr refresh
            exp2_cross.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp2_cross, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp2_cross.started')
            exp2_cross.setAutoDraw(True)
        if exp2_cross.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > exp2_cross.tStartRefresh + 0.25-frameTolerance:
                # keep track of stop time/frame for later
                exp2_cross.tStop = t  # not accounting for scr refresh
                exp2_cross.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp2_cross.stopped')
