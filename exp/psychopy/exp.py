...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/gabmac/Documents/RU/QP/exp/psychopy/untitled.py',
...
exp1_trial_image = visual.ImageStim(
    win=win,
    name='exp1_trial_image', 
    image='sin', mask=None, anchor='center-left',
    ori=0.0, pos=(0, 0), size=(0.5, 0.5),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
exp1_trial_sound = sound.Sound(num, secs=-1, stereo=True, hamming=True,
    name='exp1_trial_sound')
exp1_trial_sound.setVolume(1.0)
key_resp = keyboard.Keyboard()
exp1_trial_image__alt = visual.ImageStim(
    win=win,
    name='exp1_trial_image__alt', 
    image=image_alt, mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(0.5, 0.5),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
exp1_trial_loop = data.TrialHandler(nReps=0.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('/Users/gabmac/Documents/RU/QP/exp/trial/trial.xlsx'),
...
for thisExp1_trial_loop in exp1_trial_loop:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    exp1_trial_image.setImage(image)
    exp1_trial_sound.setSound(num, secs=5, hamming=True)
    exp1_trial_sound.setVolume(1.0, log=False)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    exp1_trialComponents = [exp1_trial_image, exp1_trial_sound, key_resp, exp1_trial_image__alt]
    ...
    while continueRoutine and routineTimer.getTime() < 6.0:
        ...
        # *exp1_trial_image* updates
        if exp1_trial_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp1_trial_image.frameNStart = frameN  # exact frame index
            exp1_trial_image.tStart = t  # local t and not account for scr refresh
            exp1_trial_image.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp1_trial_image, 'tStartRefresh')  # time at next scr refresh
            exp1_trial_image.setAutoDraw(True)
        if exp1_trial_image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > exp1_trial_image.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                exp1_trial_image.tStop = t  # not accounting for scr refresh
                exp1_trial_image.frameNStop = frameN  # exact frame index
                exp1_trial_image.setAutoDraw(False)
        # start/stop exp1_trial_sound
        if exp1_trial_sound.status == NOT_STARTED and t >= 1.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.addData('exp1_trial_sound.started', t)
            exp1_trial_sound.play()  # start the sound (it finishes automatically)
        if exp1_trial_sound.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > exp1_trial_sound.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                exp1_trial_sound.tStop = t  # not accounting for scr refresh
                exp1_trial_sound.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('exp1_trial_sound.stopped', t)
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['left-shift','right-shift'], waitRelease=False)
        ...
        # *exp1_trial_image__alt* updates
        if exp1_trial_image__alt.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp1_trial_image__alt.frameNStart = frameN  # exact frame index
            exp1_trial_image__alt.tStart = t  # local t and not account for scr refresh
            exp1_trial_image__alt.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp1_trial_image__alt, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'exp1_trial_image__alt.started')
            exp1_trial_image__alt.setAutoDraw(True)
        if exp1_trial_image__alt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > exp1_trial_image__alt.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
                exp1_trial_image__alt.tStop = t  # not accounting for scr refresh
                exp1_trial_image__alt.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exp1_trial_image__alt.stopped')
                exp1_trial_image__alt.setAutoDraw(False)
...
# completed 0.0 repeats of 'exp1_trial_loop'
