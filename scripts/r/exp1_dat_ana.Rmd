...
df_tidy %>% 
  ggplot(., aes(x = lt_std, y = rt, color = cond)) + 
    geom_point() + 
...
df_tidy %>% 
  ggplot(., aes(x = lt_std, y = rt)) + 
    geom_point() + 
    geom_abline(
      data = coef(pp_mod_cond)$participant %>% 
        as_tibble() %>% 
        rename(rt = `(Intercept)`), 
      aes(slope = lt_std, intercept = rt)) + 
    geom_abline(
      slope = fixef(pp_mod_cond)[2], 
      intercept = fixef(pp_mod_cond)[1], 
