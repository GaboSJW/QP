...
#mat_rt plot
  level_order <- c('na.es', 'f0c.es', 'f0u.es') 
  df_ltd %>% 
  ...
  labs(
    ...
    title = "Experiment 2 Matrix RT as a function of Conditions",
    caption = "Matrix RT as a function of Conditions"
  ) 
...
#cs_dur plot
  ...
  labs(
    x = "Conditions", 
    y = "CS Reaction Time", 
    title = "Experiment 2 CS RT as a function of Conditions",
    caption = "CS Reaction Time as a function of Conditions"
  ) 
...
df_ltd |> 
  filter(rt1 <= limit1 & rt2 <= limit2) %>% 
   filter(., response == '["lshift","rshift"]' |
         response == "['lshift', 'rshift']") |> 
  ggplot() + 
  aes(x = lextale_z, y = rt_cs, color = interaction(lang, type)) + 
...
df_mod_mat <- df_ltd |> 
  ...
  mutate(
    log_rt_mat = log(rt_mat), 
    condition = fct_relevel(condition, "es-na")
  ) 

mod_lt_mat <- lmer(
  log_rt_mat ~ lextale_z * condition + (1 | participant), 
  data = df_mod_mat
)
...
mod_lt_cs <- lmer(
  log_rt_cs ~ lextale_z * condition + (1 | participant), 
  data = df_mod_cs
)
...
```{r, 'mod_null'}
# Fit a null model
mod_null <- lmer(log_rt_mat ~ 1+ (1 | participant), data = df_mod_mat)
summary(mod_null)

# Add a continuous predictor

mod_1 <- lmer(log_rt_mat ~ lextale_z + (1 | participant), data = df_mod_mat)
summary(mod_1)

# Add a categorical covariate
mod_2 <- lmer(log_rt_mat ~ lextale_z +condition + (1 | participant), data = df_mod_mat)
summary(mod_2)

# Include an interaction term
mod_3 <- lmer(log_rt_mat ~  lextale_z * condition  + (1 | participant) , data = df_mod_mat)
summary(mod_3)
...
```{r, 'mod_null'}
# Fit a null model
mod_null <- lmer(log_rt_cs ~ 1+ (1 | participant), data = df_mod_cs)
summary(mod_null)

# Add a continuous predictor

mod_1 <- lmer(log_rt_cs ~ lextale_z + (1 | participant), data = df_mod_cs)
summary(mod_1)

# Add a categorical covariate
mod_2 <- lmer(log_rt_cs ~ lextale_z + condition + (1 | participant), data = df_mod_cs)
summary(mod_2)

# Include an interaction term
mod_3 <- lmer(log_rt_cs ~  lextale_z * condition + (1 | participant) , data = df_mod_cs)
summary(mod_3)
